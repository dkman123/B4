...
b4_version = '0.2'
b4_version_info = getB4versionInfo()
...
BUILD_PATH = os.path.join(BUILD_DIR, 'b4-%s-%s-%s%s' % (b4_version, BUILD_TIME, b4_version_info[1], b4_version_info[2]))  # frozen distribution path

settings = {
    'nt': {
        'binary_name': 'b4.exe',
        'icon': os.path.join(PROJECT_DIR, 'installer/assets_common', 'b4.ico'),
    },
    'darwin': {
        'binary_name': 'b4',
        'icon': os.path.join(PROJECT_DIR, 'installer/assets_common', 'b4.icns'),
    },
    'linux': {
        'binary_name': 'b4',
        'icon': None,
...
class CleanCommand(setuptools.Command):
    ...
    def run(self):
        ...
        _egg_info_dir = os.path.join(PROJECT_DIR, 'b4.egg-info')
...
class my_egg_info(orig_egg_info):
    """
    Override egg_info command to copy the b4.egg-info/PKG-INFO file into the b4 directory
    """

    def run(self):
        orig_egg_info.run(self)
        shutil.copy('b4.egg-info/PKG-INFO', 'b4/PKG-INFO')
...
if len(sys.argv) == len(set(sys.argv) - {'build_exe'}):
    ...
    class my_build_exe(build_exe):
        """extends the build_exe command to:
           ...
           - add option 'linux_binary_name' to define the name of the B4 binary file
           - produce a zip file
        ...
        user_options.extend([('linux-binary-name=', None, "file name of the Linux B4 binary [default: b4_run]")])
        ...
        def run(self):
            ...
            current_b4_version_part1, current_b4_version_part2 = self.get_version()
            self.clean_compiled_files()
            self.chmod_exec()
            self.unix2dos()
            release_name = 'b4-%s%s-%s%s' % (current_b4_version_part1, current_b4_version_part2, b4_version_info[1], b4_version_info[2])

            self.make_zip(release_name)
            self.make_innosetup(current_b4_version_part1, current_b4_version_part2)

        def get_version(self):
            """extract version number from the b4.egg-info/PKG-INFO file"""
            log.info(">>> parse B4 version")
            ...
            current_b4_version_part1 = match.group("numbers")
            current_b4_version_part2 = ""
            if match.group("pre_release"):
                current_b4_version_part2 += match.group("pre_release")
            if match.group("suffix"):
                current_b4_version_part2 += match.group("suffix")

            return current_b4_version_part1, current_b4_version_part2
        ...
        def make_zip(self, release_name):
            ...
            for root, dirs, files in os.walk(self.build_exe):
                for filename in files:
                    path = os.path.abspath(os.path.join(root, filename))
                    zipf.write(path, arcname=os.path.join('b4', path[len(self.build_exe):]))
            zipf.close()

        def make_innosetup(self, current_b4_version_part1, current_b4_version_part2):
                ...
                for filename in yaml_config['innosetup_scripts']:
                    ...
                    try:
                        cmd = [
                            ...
                            script_file, '/Q',
                            '/O' + DIST_DIR,
                            '/dB4_VERSION_NUMBER=' + current_b4_version_part1,
                            '/dB4_VERSION_SUFFIX=' + current_b4_version_part2,
                            '/dB4_VERSION_PLATFORM=' + b4_version_info[1],
                            '/dB4_VERSION_ARCHITECTURE=' + b4_version_info[2],
                            '/dB4_BUILD_PATH=' + self.build_exe,
                        ]
                        subprocess.call(cmd)
...
setup(
    ...
    description="BigBrotherBot (B4) is a cross-platform, cross-game game administration bot. "
    ...
    long_description="Big Brother Bot B4 is a complete and total server administration package for online games. "
                     "B4 is designed primarily to keep your server free from the derelicts of online gaming, but offers "
                     "more, much more. With the stock configuration files, B4 will will keep your server free from "
                     ...
                     "and ultimately kick, and or ban them for a predetermined time limit. B4 was designed to be easily "
                     "ported to other online games. Currently, B4 is in production for the Call of Duty series, "
                     ...
                     "provide much of the functionality for B4. These plugins can easily be configured. An SDK will "
    ...
    options={
        ...
        'bdist_mac': {
            ...
            'bundle_name': 'BigBrotherBot (B4) %s' % b4_version,
        },
        'bdist_dmg': {
            'dist_dir': DIST_DIR,
            'volume_label': 'BigBrotherBot (B4) %s' % b4_version,
        ...
        'build_exe': {
            ...
            'include_files': [
                ('README.md', 'README.md'),
                ('b4/PKG-INFO', 'PKG-INFO'),
                ('b4/docs/', 'docs'),
                ('b4/conf', 'conf'),
                ('b4/conf/templates', 'conf/templates'),
                ('b4/conf/templates/autodoc', 'conf/templates/autodoc'),
                ('b4/extplugins/', 'extplugins'),
                ('b4/plugins/', 'plugins/'),
                ('b4/sql', 'sql'),
                ('b4/sql/mysql', 'sql/mysql'),
                #('b4/sql/postgresql', 'sql/postgresql'),
                #('b4/sql/sqlite', 'sql/sqlite'),
                ('examples', 'examples'),
                ('scripts', 'scripts'),
            ],
        }

...
class ExamplePlugin(b4_plugin.Plugin):
    """
    This class is meant to ease plugin development.
    Plugin developers who are not familiar with B4 core can copy this plugin module (the whole 'example'
    folder in 'examples/plugins' is the plugin module) over and make their own changes.
    This class illustrates basic interactions with the B4 core: in particular it shows the following:

        - defining new commands
        - reacting on B4 events
        - schedule execution of functions
        - client dynamic variable management (no persistence)

    If you want to see the outcome of this example you can load the plugin in your B4 to test it:

        - move the example folder inside @b4/extplugins
        - add the plugin entry to your main configuration file:

                - b4.ini => example: @b4/extplugins/example/conf/plugin_example.ini
                - b4.xml => <plugin name="example" config="@b4/extplugins/example/conf/plugin_example.ini" />
    ...
    >>> self.config ## Plugin configuration file (b4.config.CfgConfigParser or b4.config.XmlConfigParser instance)
    >>> self.console ## B4 parser instance (inherits from b4.parser.Parser)
    >>> self.console.clients ## b4.clients.Clients instance (to retrieve client information)
    >>> self.console.cron ## b4.cron.Cron instance (for scheduled execution)
    >>> self.console.storage ## b4.storage.Storage instance (to communicate with the storage layer)
    >>> self.console.config ## b4.config.MainConfig instance (the B4 main configuration file)
    ...
    >>> self.critical('message')  ## Log a CRITICAL message (B4 will stop)
    """
    ...
    # file path in your B4 main configuration file when loading the plugin.
    requiresConfigFile = True

    # Set this to the minimum required version of B4 needed to run your plugin.
    # If you omit this value, your plugin will be loaded no mater the version of the B4 currently running.
    ...
    # The Geolocation plugin (in b4.plugins.geolocationX) produces 2 new events: EVT_GEOLOCATION_SUCCESS and
    # EVT_GEOLOCATION_FAILURE. If your plugin needs to react on those  events, you need the geolocationX plugin
    # to be loaded, and the following line would have been: requiresPlugins = ['geolocationX']. B4 will then attempt
    # to load the geolocationX plugin (even if the user did not specified such plugin in the B4 main configuration
    # file). If the geolocationX plugin can't be loaded, also your plugin won't be loaded since a 'strong' requirement
    ...
    def __init__(self, console, config=None):
        ...
        # constructor will sto B4 from loading your plugin, i.e: here below we retrieve the Admin plugin: if this
        # somehow fails, an AttributeError exception is raised which will prevent the plugin to B4 loaded by B4
        # leaving some message in the B4 log file explaining the reason why the plugin is not being loaded.
        self.adminPlugin = self.console.getPlugin('admin')
        if not self.adminPlugin:
            raise AttributeError('could not load admin plugin: this message will be printed in the B4 log file')
        ...
        # This will be set later in onStartup and it's used by the b4.cron.Cron object to schedule function execution.
        self.crontab = None
        """:type: b4.cron.PluginCronTab"""
    ...
    def onStartup(self):
        ...
        # be named accordingly to the command name, i.e: the command !hello, whose name is 'hello' is implemented
        ...
        #
        # You can find B4 default events inside b4.events. Some events are bounded to specifid games and they
    ...
    def scheduled_refill(self):
        ...
        for client in self.console.clients.getList():
            ...
            client.message('B4 gifted you 1 cookie: you now have %s cookies' % client_cookies)
    ...
    def cmd_hello(self, data, client, cmd=None):
        ...
        :param cmd: the command instance (see b4_plugins.admin.Command)
        ...
        if not data:
            ...
            # also the reference of the object of the client who issued the command. In this way if no
    ...
    def cmd_cookie(self, data, client, cmd=None):
        ...
        :param client: the client who executed the command
        :param cmd: the command instance (see b4.plugins.admin.Command)

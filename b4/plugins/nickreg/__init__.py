...
class NickregPlugin(b4.b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        """
        self.min_level = self.getSetting('settings', 'min_level', b4.LEVEL, 20)
        self.min_level_global_manage = self.getSetting('settings', 'min_level_global_manage', b4.LEVEL, 100, lambda x: clamp(x, minv=self.min_level))
        self.max_nicks = self.getSetting('settings', 'max_nicks', b4.INTEGER, 3)
        self.interval = self.getSetting('settings', 'interval', b4.INTEGER, 30)
    ...
    def onStartup(self):
        ...
        if 'nicks' not in self.console.storage.getTables():
            sql_path_main = b4.getAbsolutePath('@b4/plugins/nickreg/sql')

...
class AdminPlugin(b4.b4_plugin.Plugin):
    ...
    def onStartup(self):
        ...
        if not self.console.autorestart:
            if 'restart' in self._commands:
                self.debug('unregistering !restart command: b4 is not running in autorestart mode')
    ...
    def cmd_b4(self, data, client, cmd=None):
        ...
        if len(data) > 0 and client.maxLevel >= self._admins_level:
            ...
            if data == 'poke':
                self.console.say('^7Do not poke b4 %s^7!' % client.exactName)
            elif data == 'expose':
                self.console.say('^7Do expose b4 to sunlight %s^7!' % client.exactName)
            elif data == 'stare':
                self.console.say('^7Do not stare at b4 %s^7!' % client.exactName)
            elif data == 'stab':
                self.console.say('^7b4 is invincible, %s^7 could not penetrate he hide of b4' % client.exactName)
            elif data == 'bite':
                self.console.say('^7b4 breaks %s^7\'s teeth with its metalic frame' % client.exactName)
            elif data == 'fuck':
                self.console.say('^7b4 doesn\'t need your hand me out %s^7' % client.exactName)
            elif data == 'slap':
                self.console.say('^7%s^7 is not Rick James' % client.exactName)
            elif data == 'fight':
                self.console.say('^7%s^7 is knocked out by b4' % client.exactName)
            elif data == 'feed':
                self.console.say('^7b4 enjoys your nourishment %s^7' % client.exactName)
            elif data == 'throw':
                self.console.say('^7b4 can fly %s^7, and you throw like a sissy' % client.exactName)
            elif data == 'furniture':
                self.console.say('^7b4 does make a lovely lamp %s^7' % client.exactName)
            elif data == 'indeed':
                ...
                self.console.say('^7b4 has been known to cause lung cancer when smoked %s^7' % client.exactName)
            elif data == 'maul':
                self.console.say('^7b4 casts a spell of invisibility, you can\'t find  %s^7' % client.exactName)
            elif data == 'procreate':
                self.console.say('^7b4 2.0 will soon be on the way %s^7' % client.exactName)
            elif data == 'shoot':
                self.console.say('^7Your hit detection is off %s^7, b4 is unharmed' % client.exactName)
            elif data == 'kick':
    ...
    def cmd_putgroup(self, data, client, cmd=None):
        ...
        m = re.match(r'^(.{2,}) ([a-z][a-z0-9]+)$', data, re.I)
        if not m:
            client.message(self.getMessage('invalid_parameters'))
            return
                ...
                if client.maxLevel >= self._admins_level:
                    ...
                    if sclient:
                        if sclient.maxLevel >= client.maxLevel:
                            ...
                            threading.Thread(target=self._sendRules, args={'sclient': sclient}).start()
                else:
                    client.message('^7Stop trying to spam other players')
            else:
                if cmd.loud or cmd.big:
                    threading.Thread(target=self._sendRules, kwargs={'sclient': None, 'big': cmd.big}).start()
                else:
                    threading.Thread(target=self._sendRules, kwargs={'sclient': client}).start()
    ...
    def cmd_tempban(self, data, client=None, cmd=None):
        ...
        cid = m[0]
        # r before the string represents a "raw string"
    ...
    def cmd_poke(self, data, client=None, cmd=None):
        ...
        if not m:
            ...
            if m[0] == 'b4':
                self.warnClient(client, 'Do not poke b4!', None, False, '', 1)
            else:
                sclient = self.findClientPrompt(m[0], client)
                if sclient:

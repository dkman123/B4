...
class CallvotePlugin(b4.b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        if self.config.has_section('callvoteminlevel'):
            for o in self.config.options('callvoteminlevel'):
                self.callvoteminlevel[o] = self.getSetting('callvoteminlevel', o, b4.LEVEL, self.callvoteminlevel[o])

        if self.config.has_section('callvotespecialmaplist'):
            for o in self.config.options('callvotespecialmaplist'):
                self.callvotespecialmaplist[o.lower()] = self.getSetting('callvotespecialmaplist', o.lower(), b4.LEVEL)
    ...
    def onStartup(self):
        ...
        if 'callvote' not in self.console.storage.getTables():
            sql_path_main = b4.getAbsolutePath('@b4/plugins/callvote/sql')

...
class SpawnkillPlugin(b4.b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        for index in ('hit', 'kill'):
            self.settings[index]['maxlevel'] = self.getSetting(index, 'maxlevel', b4.LEVEL, self.settings[index]['maxlevel'])
            self.settings[index]['delay'] = self.getSetting(index, 'delay', b4.INT, self.settings[index]['delay'])
            self.settings[index]['duration'] = self.getSetting(index, 'duration', b4.DURATION, self.settings[index]['duration'])
            self.settings[index]['reason'] = self.getSetting(index, 'reason', b4.STR, self.settings[index]['reason'])
            self.settings[index]['penalty'] = self.getSetting(index, 'penalty', b4.STR, self.settings[index]['penalty'], validate)

...
class MakeroomPlugin(Plugin):
    ...
    connected player from the lowest b4 group
    """
    ...
    def onClientAuth(self, event):
        ...
        if self._retain_free_slot_info is not None and time.time() > self._retain_free_slot_info['until']:
            self._retain_free_slot_info = None
    ...
    def _kick(self, non_member, admin):
        ...
        :param non_member: b4.clients.Client
        """
        try:

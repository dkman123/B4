...
class JumperPlugin(b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        """
        self._demo_record = self.getSetting('settings', 'demorecord', b4.BOOL, self._demo_record)
        self._skip_standard_maps = self.getSetting('settings', 'skipstandardmaps', b4.BOOL, self._skip_standard_maps)
        self._min_level_delete = self.getSetting('settings', 'minleveldelete', b4.LEVEL, self._min_level_delete)
    ...
    def onStartup(self):
        ...
        if self._demo_record:
            ...
            if not self.serverSideDemoPlugin:
                ...
                self.warning("automatic demo recording is enabled in configuration file but plugin 'urtserversidedemo' "
                             "has not been loaded in b4 main configuration file: automatic demo recording will be disabled")
        ...
        if 'jumpruns' not in current_tables or 'jumpways' not in current_tables:
            sql_path_main = b4.getAbsolutePath('@b4/plugins/jumper/sql')
    ...
    def onMapChange(self, event):
        ...
        thread = Timer(interval=30.0, function=self.welcomeClients, args=(event.data['new'].lower(),))
        thread.daemon = True
        thread.start()
    ...
    def getMapsDataFromApi(self):
        ...
        try:
            ...
            data = {d['pk3'].lower(): d for d in resp}

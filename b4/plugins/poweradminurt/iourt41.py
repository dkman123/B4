...
class Poweradminurt41Plugin(b4.b4_plugin.Plugin):
    ...
    def loadPublicMode(self):
        ...
        try:
            ...
            padic = self.getSetting('publicmode', 'usedic', b4.BOOL, False)
            if padic:
        ...
        self._papublic_password = self.getSetting('publicmode', 'g_password', b4.STR, None)
    ...
    def loadMatchMode(self):
        ...
        """
        self._match_plugin_disable = self.getSetting('matchmode', 'plugins_disable', b4.LIST, [])
    ...
    def _getAvgKillsRatios(self, blue, red):
        ...
        blue = sorted(blue, key=lambda Client: Client.id)
        red = sorted(red, key=lambda Client: Client.id)
...
#   shuffleteams
    ...
    def cmd_paslap(self, data, client, cmd=None):
        ...
        if args[1]:
            ...
            if x in range(1, 26):
                threading.Thread(target=self.multipunish, args=(x, sclient, client, 'slap')).start()
            else:
    ...
    def cmd_panuke(self, data, client, cmd=None):
        ...
        if args[1]:
            ...
            if x in range(1, 26):
                threading.Thread(target=self.multipunish, args=(x, sclient, client, 'nuke')).start()
            else:
                client.message('^7Number of punishments out of range, must be 1 to 25')
        else:
    ...
    def cmd_pasetnextmap(self, data, client=None, cmd=None):
        ...
        if not data:
            ...
            if isinstance(match, str):
                ...
                client.message('do you mean : %s ?' % match[:5].join(', '))
            else:

...
class Ranges:
    ...
    # the list.
    # We use a binary search algorithm for speed on long
    ...
    def addrange(self, start, end):
        ...
        if i == len(self._l):
            ...
            if start >= r[0] and end <= r[1]:
                return
            # glue it in order; right now, ordered by start.
        ...
        while i < len(self._l):
            ...
            if r[0] - 1 <= ro[1]:
                ...
                # if we have done no work now, we will never do any.
                break
    ...
    def __contains__(self, val):
        ...
        return ((p < len(self._l) and (self._l[p][0] <= val <= self._l[p][1])) \
                or (pm >= 0 and (self._l[pm][0] <= val <= self._l[pm][1])))
    ...
    # into a long. We try to coerce any inadvertent longs into ints;

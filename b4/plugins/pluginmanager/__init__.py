    ...
    def do_load(self, client, name=None):
        ...
        def _get_plugin_config(p_name, p_clazz):
            ...
            def _search_config_file(match):
                ...
                search = '%s%s*%s*' % (b4.getAbsolutePath('@b4\\conf'), os.path.sep, match)
                ...
                search = '%s%s*%s*' % (os.path.join(b4.getAbsolutePath(extplugins_dir), match, 'conf'), os.path.sep, match)
                self.debug('searching for configuration file(s) matching: %s' % search)
                collection = glob.glob(search)
                return collection
        ...
        try:
            ...
            client.message('^7Please put the plugin module in ^3@b4/extplugins/')
            ...
            def _get_plugin_data(p_data):
                ...
                if p_data.clazz.requiresVersion and B4version(p_data.clazz.requiresVersion) > B4version(currentVersion):
                    raise MissingRequirement('plugin %s requires b4 version %s (you have version %s) : please update your '
                                             'b4 if you want to run this plugin' % (p_data.name, p_data.clazz.requiresVersion, currentVersion))

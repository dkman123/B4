...
class FtpytailPlugin(b4.b4_plugin.Plugin):
    ...
    def onStartup(self):
        ...
        self.lgame_log = b4.getWritableFilePath(self.lgame_log)
    ...
    def onLoadConfig(self):
        ...
        """
        # These values go in b4 xml, so check them first
        try:
            ...
            self.debug('could not find server/use_windows_cache_fix in b4 config file, '
                       'using default: %s' % self._use_windows_cache_fix)
        except ValueError as e:
            self.error('could not load server/use_windows_cache_fix config value from b4 config file: %s' % e)
        ...
        if self._use_windows_cache_fix:
            ...
            try:
                ...
                self.debug('could not find server/cache_refresh_delay in b4 config file, '
                         'using default: %s' % self._cache_refresh_delay)
            except ValueError as e:
                self.error('could not load server/cache_refresh_delay config value from b4 config file: %s' % e)
    ...
    def update(self):
        ...
        self.file.write('\r\n'.encode("utf-8"))
        self.file.write('b4 has been restarted\r\n'.encode("utf-8"))
        self.file.write('\r\n'.encode("utf-8"))
            ...
            try:
                ...
                if self._logAppend:
                    try:
                        self.file = open(self.lgame_log, 'ab')
                        self.file.write('\r\n'.encode("utf-8"))
                        self.file.write('b4 has restarted writing the log file\r\n'.encode("utf-8"))
                        self.file.write('\r\n'.encode("utf-8"))
                    except IOError:
...
if __name__ == '__main__':
    ...
    <configuration plugin="ftpytail">
        <settings name="settings">
            ...
            <!-- The 3 settings below defines how aggressive will be b4 at 

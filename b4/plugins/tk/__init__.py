...
class TkPlugin(b4.b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        """
        self._issue_warning = self.getSetting('settings', 'issue_warning', b4.STR, self._issue_warning)
        self._round_grace = self.getSetting('settings', 'round_grace', b4.INT, self._round_grace)
        self._maxPoints = self.getSetting('settings', 'max_points', b4.INT, self._maxPoints)
        self._private_messages = self.getSetting('settings', 'private_messages', b4.BOOL, self._private_messages)
        self._damage_threshold = self.getSetting('settings', 'damage_threshold', b4.INT, self._damage_threshold)
        self._tk_warn_duration = self.getSetting('settings', 'warn_duration', b4.STR, self._tk_warn_duration)
        self._warn_level = self.getSetting('settings', 'warn_level', b4.INT, self._warn_level)
        self._tkpointsHalflife = self.getSetting('settings', 'halflife', b4.INT, self._tkpointsHalflife)
        self._grudge_enable = self.getSetting('settings', 'grudge_enable', b4.BOOL, self._grudge_enable)
        self._grudge_level = self.getSetting('settings', 'grudge_level', b4.INT, self._grudge_level)

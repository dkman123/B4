...
class TranslatorPlugin(b4.b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        self.settings['default_source_language'] = self.getSetting('settings', 'default_source_language', b4.STR, 'auto', validate_source)
        self.settings['default_target_language'] = self.getSetting('settings', 'default_target_language', b4.STR, 'en', validate_target)
        self.settings['display_translator_name'] = self.getSetting('settings', 'display_translator_name', b4.BOOL, True)
        self.settings['translator_name'] = self.getSetting('settings', 'translator_name', b4.STR, '^7[^1T^7]')
        self.settings['min_sentence_length'] = self.getSetting('settings', 'min_sentence_length', b4.INT, 6)
        self.settings['min_time_between'] = self.getSetting('settings', 'min_time_between', b4.INT, 30)
        self.settings['always_loud'] = self.getSetting('settings', 'always_loud', b4.BOOL, True)
        self.settings['max_history'] = self.getSetting('settings', 'max_history', b4.INT, 10)
        self.settings['exclude_language'] = self.getSetting('settings', 'exclude_language', b4.STR, 'en', validate_target)

...
import sys
import shutil
#import urllib2 # change to urllib
import zipfile
...
def main_is_frozen():
    """
    Detect if B4 is running from compiled binary.
    """
...
def confirm(client):
    """
    Used to identify B4 developers.
    """
    msg = 'No confirmation...'
    ## SKIPPED
    # try:
    #     # First test against known guids
...
def clamp(value, minv=None, maxv=None):
    """
    Clamp a value so that it's bounded within min and max
...
def topological_sort(source):
    ...
    :param source: list of ``(name, set(names of dependencies))`` pairs
    """
    ...
    while pending:
        ...
        if not next_emitted: # all entries have unmet deps, one of two things is wrong...
            raise ProgrammingError("cyclic or missing dependency detected: %r" % (next_pending,))

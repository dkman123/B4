...
class DatabaseStorage(Storage):
    ...
    def __call__(self, *args, **kwargs):
        if self not in self._instances:
            with self._lock:
                if self not in self._instances:
                    self._instances[self] = super(DatabaseStorage, self)
    ...
    def getClient(self, client):
        ...
        try:
            ...
            cursor = self.query(b4.b4_querybuilder.QueryBuilder(self.db).SelectQuery(
            ...
            # self.console.warning("NOISY getClient %s; id %s getting app %s; new %s"
            # % (client.name, client.id, client.app, saveApp))
    ...
    def getClientsMatching(self, match):
        ...
        self.getConnection()
        cursor = self.query(
            b4.b4_querybuilder.QueryBuilder(self.db).SelectQuery(
    ...
    def setClient(self, client):
        ...
        self.console.debug("NOISY %s setClient saving id %s; app %s; isocode %s; permmute %s" % (
            client.name, client.id, client.app, client.isocode, client.permmute))
        ...
        if client.id > 0:
            self.query(b4.b4_querybuilder.QueryBuilder(self.db).UpdateQuery(
                data, 'clients', {'id': client.id}))
        else:
            cursor = self.query(b4.b4_querybuilder.QueryBuilder(
                self.db).InsertQuery(data, 'clients'))
    ...
    def setClientAlias(self, alias):
        ...
        if alias.id:
            ...
            cursor = self.query(b4.b4_querybuilder.QueryBuilder(self.db).InsertQuery(
                data, 'aliases'))
    ...
    def getClientAlias(self, alias):
        ...
        if hasattr(alias, 'id') and alias.id > 0:
            query = b4.b4_querybuilder.QueryBuilder(self.db).SelectQuery(
    ...
    def numPenalties(self, client, penType='Ban'):
        ...
        :param penType: The penalties type.
    ...
    def _query(self, query, bindata=None):
        ...
        #if not dbcursor:
        #    self.console.info("b4_common _query says dbcursor looks BAD!")
        return dbcursor
    ...
    def query(self, query, bindata=None):
        ...
        if not connection:
            self.console.error("b4_common lost connection with the storage layer during query")
            raise Exception('lost connection with the storage layer during query')

        try:
            #self.console.info("b4_common calling _query")

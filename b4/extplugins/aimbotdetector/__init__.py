...
class AimbotdetectorPlugin(b4.b4_plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        self.console.debug('Players with level %s and above will not be checked' % self.ignorelevel)
        ...
        self.console.debug('Aimbot Detector Plugin is set to %s' % self._actions[self.action])

        if self.action == 1:
            try:
                self.duration = self.config.get('settings', 'duration')
                self.console.debug('Tempban duration is: %s' % self.duration)
            except:
                self.duration = '2h'
                self.console.debug('Using default tempban duration %s' % self.duration)
        ...
        try:
            ...
            self.console.debug('Cannot load e-mail option, disabling...')
        ...
        if self.mailtoadmin:
            try:
                ...
                self.console.debug('Cannot load e-mail settings, please check your config file. Disabling e-mailing feature...')
                self.mailtoadmin = False

        self.console.debug('Starting')
    ...
    def checkHitlocKills(self, client=None, victim=None, damage_location=None):
        ...
        # client (attacker)
        if client:
            self.console.debug('%s has a kill hit in %s' % (client.name, damage_location))
            ...
            if damage_location in self._hitlocs:
                ...
                self.console.debug('%s has a %s kill streak for monitored body parts' % (client.name, HitlocStats.hitloc_kills))
    ...
    def addFollow(self, sclient):
        ...
        if cursor.rowcount == 0:
            ...
            cursor2.close()
            self.console.debug("Suspect added to follow plugins watch list")
        else:
            self.console.debug("Suspect already in follow plugins watch list")
        cursor.close()
    ...
    def mail2Admins(self, client=None):
        ...
        self.console.debug('Sending alert e-mail to %s' % receivers)
        ...
        try:
            ...
            self.console.debug('Error: unable to send e-mail: %s' % err)
...
if __name__ == '__main__':
    ...
    from b4_fake import joe, simon, moderator, superadmin

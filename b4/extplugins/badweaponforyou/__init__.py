...
class BadweaponforyouPlugin(b4.b4_plugin.Plugin):
    ...
    def onStartup(self):
        ...
        if self.gamename == 'iourt41':
            self.gmessage = 'gear[beretta|de|spas|mp5|ump|hk|lr300|g36|psg1|sr8|ak|negev|he|smoke' \
                            + '|kevlar|helmet|silencer|laser|medkit|tag|xtra]'

        if self.gamename == 'iourt42':
            self.gmessage = 'gear[beretta|de|glock|colt|spas|mp5|ump|mac11|hk|lr300|g36|psg1|sr8|ak|negev|he|smoke' \
                            + '|kevlar|helmet|silencer|laser|medkit|tag|xtra]'

        if self.gamename == 'iourt43':
            self.gmessage = 'gear[beretta|de|glock|colt|spas|mp5|ump|mac11|hk|lr300|g36|psg1|sr8|ak|negev|he|smoke' \
                            + '|kevlar|helmet|silencer|laser|medkit|tag|xtra|frf1|benelli|p90|magnum]'
    ...
    def cmd_bwfy(self, data, client, cmd=None):
        ...
        if data:
            inputInfo = self._adminPlugin.parseUserCmd(data)

        else:
            client.message('!bwfy <playername> <on or off> <gear>')
            return

        sclient = self._adminPlugin.findClientPrompt(inputInfo[0], client)
        ...
        if not inputInfo[1]:
            ...
            return False
        nespace = inputInfo[1].count(' ')

        if nespace == 0:
            client.message('!bwfy <playername> <on or off> <gear> (missing on|off or gear specification)')
            return False

        tdata = inputInfo[1].split(' ')
        onoff = tdata[0]
        sgear = tdata[1]
        sayonoff = ""
            ...
            if lclient.id == -1:  # "all"
                ...
                for x in self._listplayersgear:
                    ...
                    for gear in self._listplayersgear[x]:
                        client.message('^3Weapon /gear prohibited for ^2%s ^3: ^7-%s-' % (sdclient.exactName
                                                                                          , self._saysgear[gear]))
                ...
                try:
                    ...
                    for gear in self._listplayersgear[lclient.id]:
                        client.message('^3Weapon /gear prohibited for %s ^3: ^7-%s-' % (lclient.exactName
                                                                                        , self._saysgear[gear]))
